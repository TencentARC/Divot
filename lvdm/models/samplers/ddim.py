...
class DDIMSampler(object):
    def __init__(self, model, num_timesteps=1000, schedule="linear", **kwargs):
        super().__init__()
        self.model = model
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0., verbose=True):
        ...
        alphas_cumprod = self.model.alphas_cumprod.to(torch.float16)
    ...
    def ddim_sampling(self, cond, shape,
        ...
        if precision is not None:
            if precision == 16 or precision == 'bf16':
        ...
        for i, step in enumerate(iterator):
            ...
            #print(x0.shape, clean_cond, mask)
    ...
    def predict_eps_from_z_and_v(self, x_t, t, v):
        return (
                extract_into_tensor(self.model.sqrt_alphas_cumprod.to(t.device), t, x_t.shape) * v +
                extract_into_tensor(self.model.sqrt_one_minus_alphas_cumprod.to(t.device), t, x_t.shape) * x_t
        )

    def predict_start_from_z_and_v(self, x_t, t, v):
        return (
                extract_into_tensor(self.model.sqrt_alphas_cumprod.to(t.device), t, x_t.shape) * x_t -
                extract_into_tensor(self.model.sqrt_one_minus_alphas_cumprod.to(t.device), t, x_t.shape) * v
        )
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            model_output = self.model(x, t, context=c, **kwargs)
        else:
            ### do_classifier_free_guidance
            if isinstance(c, torch.Tensor) or isinstance(c, dict):
                e_t_cond = self.model(x, t, context=c, **kwargs)
                e_t_uncond = self.model(x, t, context=unconditional_conditioning, **kwargs)
            else:

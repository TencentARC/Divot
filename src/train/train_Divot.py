...
class TrainingArguments:
    ...
    num_frames: int = field(default=4, metadata={"help": "Number of frames per video clip"})
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(agent_model):
                with torch.no_grad():
                    ...
                    bz = frames.shape[0]
                    frames = frames.reshape(-1, args.num_frames, frames.shape[2], frames.shape[3], frames.shape[4])
                    video_embeds = visual_encoder(frames)
                    video_embeds = video_embeds.reshape(bz, -1, video_embeds.shape[-2], video_embeds.shape[-1])
                    torch.cuda.empty_cache()
